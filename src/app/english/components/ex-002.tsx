...
// src/app/english/components/ex-002.tsx
...
const someAny = { correct: ['some', 'any'] };
const someNil = { correct: ['some', ''] };
const anyNil  = { correct: ['any', ''] };
const aNil    = { correct: ['a', ''] };
const theNil  = { correct: ['the', ''] };
const my      = { correct: ['my'] };
const your    = { correct: ['your'] };
const his     = { correct: ['his'] };
const her     = { correct: ['her'] };
const its     = { correct: ['its'] };
...
const Exercise002 = ({ accessStatus }: { accessStatus: AccessStatus }) => {
    ...
    return ex.parts.map((part, i) => (
      <span key={i}>
        ...
        {i < ex.blanks.length && (
            ...
            placeholder=". . . . . . . . . ."
            disabled={isLocked}
            data-state={showResult === null ? 'typing' : ex.blanks[i].correct.includes(norm(answers[i] ?? '')) ? 'correct' : 'incorrect'}
    ...
    <div style={{ maxWidth: 680, margin: '0 auto', padding: '1.5rem 1rem 3rem', fontFamily: "'Source Serif 4', Georgia, serif", color: 'inherit' }}>
      ...
      <div style={{ borderBottom: '1px solid #ccc', paddingBottom: '1rem', marginBottom: '1.25rem', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 13, color: '#666', lineHeight: 1.6 }}>
          The use of articles · <em>the</em>, <em>a</em>, <em>an</em>, <em>some</em>, <em>any</em>, <em>no</em>, or leave the blank empty
        </div>
      ...
      <div style={{ display: 'flex', gap: 8, justifyContent: 'center', marginBottom: '1rem', flexWrap: 'wrap' }}>
        <button
          onClick={checkAnswer}
          disabled={isLocked || answers.every((a) => a === '')}
          style={{
            background: isLocked || answers.every((a) => a === '') ? '#ccc' : '#185FA5',
            border: 'none', borderRadius: 8, padding: '7px 20px',
            fontSize: 13, cursor: isLocked || answers.every((a) => a === '') ? 'not-allowed' : 'pointer',
        ...
        <button
          onClick={reset}
          disabled={isLocked}
          style={{ background: '#fff', border: '0.5px solid #ccc', borderRadius: 8, padding: '7px 20px', fontSize: 13, cursor: isLocked ? 'not-allowed' : 'pointer', color: isLocked ? '#bbb' : '#333', fontFamily: "'Source Serif 4', Georgia, serif" }}
        >
          Reset
        </button>
        <button
