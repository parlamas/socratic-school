//src/app/symposium/components/LanguageBlock.tsx
    ...
    <div className="flex items-start gap-4">
      ...
      <div className="flex-grow">
        <div className="flex items-center justify-between mb-3">
          ...
          {/* Button group at top - Swell or Retreat based on state */}
          {!isSwelled ? (
            <button
              onClick={() => onSwell(langIndex)}
              className="px-3 py-1 text-xs rounded-full transition-all bg-blue-50 text-blue-700 hover:bg-blue-100 border border-blue-200"
            >
              Swell
            </button>
          ) : (
        ...
        {isSwelled && (
          <div className="mt-4 flex items-center justify-between">
            ...
            {/* Keep the bottom Retreat button for convenience */}
            <button
              onClick={() => onRetreat(langIndex)}
              className="px-3 py-1 text-xs rounded-full transition-all bg-amber-50 text-amber-700 hover:bg-amber-100 border border-amber-200"
            >
